...
sellerWorkflow.uploadSellerProduct = function(apiname, lang, locale,request, responseCallback) {
	...
	var failureResponse = {
        	"responseCode": -1,
        	"response": "",
        	"errorMsg": "Issue in uploading product, Please try again or contact scalelabs Team"
	};
	async.waterfall(
	[
		function(callback) {
			sellerService.insertProduct(apiname,request.userName,request.productDetails,function(insertProductResponse) {
				if(insertProductResponse.responseCode === 0) {
					sucessResponse.errorMsg="Products Uploaded successfully!!";
...
//Delete the older product
sellerWorkflow.uploadSellerProductOld = function(apiname, lang, locale,request, responseCallback) {
	logger.info(request.userName + ":seller_workflow:uploadSellerProduct:" + apiname + ":Enter");
	var sucessResponse = {
                "responseCode": 0,
                "response": {},
...
sellerWorkflow.updateProduct = function(apiname, lang, locale,request, responseCallback) {
	logger.info(request.userName + ":seller_workflow:updateProduct:" + apiname + ":Enter");
	async.waterfall(
	[
		function(callback) {
			sellerService.updateProduct(apiname,request.userName,request.productDetails,function(updateProductResponse) {
				if(updateProductResponse.responseCode === 0) {
                                        callback(null,updateProductResponse);
                                } else {
	...
	function(err, results) {
                if (err) {
                        logger.error(request.userName + ":seller_workflow:updateProduct:" + apiname + ":Exit",err);
                        responseCallback(err);
                } else {
                        logger.info(request.userName + ":seller_workflow:updateProduct:" + apiname + ":Exit",results);
                        responseCallback(results);
                }
        });
};
                ...
                function(callback) {
                        sellerService.getAllProducts(apiname,request.userName,function(getAllProductsResponse) {
                                if(getAllProductsResponse.responseCode === 0) {
                                        ...
                                        failureResponse.errorMsg = "Issue in retriving the products, Please try again or contact scalelabs support team!!";
                                        callback(failureResponse,null);
                                }
                        });
                },
...
sellerWorkflow.getProductLevelInventoryReport = function(apiname, lang, locale, request, responseCallback) {
    logger.info(request.userName + ":seller_workflow:getProductLevelInventoryReport:" + apiname + ":Enter");
    async.waterfall(
        [
            function(callback) {
                sellerService.getProductLevelInventoryReport(apiname, request.userName, function(getProductLevelInventoryResponse) {
                    if (getProductLevelInventoryResponse.responseCode === 0) {
                        callback(null, getProductLevelInventoryResponse);
                    } else {
                        failureResponse.errorMsg = "Issue in retriving the products, Please try again or contact scalelabs support team!!";
                        callback(failureResponse, null);
                    }
        ...
        function(err, results) {
            if (err) {
                logger.error(request.userName + ":seller_workflow:getProductLevelInventoryReport:" + apiname + ":Exit", err);
                responseCallback(err);
            } else {
                logger.info(request.userName + ":seller_workflow:getProductLevelInventoryReport:" + apiname + ":Exit", results);
                responseCallback(results);
            }
        });
};

sellerWorkflow.getTopProductsSold = function(apiname, lang, locale, request, responseCallback) {
    logger.info(request.userName + ":seller_workflow:getTopProductsSold:" + apiname + ":Enter");
    async.waterfall(
        [
            function(callback) {
                sellerService.getTopProductsSold(apiname, request.userName, function(getTopProductsSoldResponse) {
                    if (getTopProductsSoldResponse.responseCode === 0) {
                        callback(null, getTopProductsSoldResponse);
                    } else {
                        failureResponse.errorMsg = "Issue in retriving the products, Please try again or contact scalelabs support team!!";
                        callback(failureResponse, null);
                    }
        ...
        function(err, results) {
            if (err) {
                logger.error(request.userName + ":seller_workflow:getTopProductsSold:" + apiname + ":Exit", err);
                responseCallback(err);
            } else {
                logger.info(request.userName + ":seller_workflow:getTopProductsSold:" + apiname + ":Exit", results);
                responseCallback(results);
            }
        });
};
